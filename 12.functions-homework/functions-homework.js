/*1. Да се състави функция, която приема като параметър масив,
...
test([1,0,5,NaN,6,'hfgifg',10,'',null,20,false,13,17,undefined]);

/*2. Да се състави функция, която приема като първи параметър масив от обекти, а като втори обект.
Функцията трябва да върне първия обект, който да съдържат пропъртитата на обекта от втория параметър. 
(_.where(arr, {prop: 123})) Ако няма такъв обект, да върне NULL.*/
...
/*3. Да се състави функция, която приема N на брой параметри и сумира тези от тях, които са от тип Number
...
function sumNumbers () {
  ...
  for (let arrVal of arr){
    if (typeof arrVal === 'number' && !Number.isNaN(arrVal)) {
      sum += arrVal;
    }
  }
  return sum;
}
sumNumbers(10,"hjdgfygf",3,null,{},false,7);
...
/*4. Да се състави функция, която приема два параметъра, които са от тип Number и връща random число между 
стойностите на двата параметъра.Добавете валидация за типа на двата параметъра и първия параметър винаги 
да е по-малък от втория.*/

//first version
function randomNum (param1,param2) {
  if ( (typeof param1 === 'number') && (!Number.isNaN(param1)) 
        && (typeof param2 === 'number') && (!Number.isNaN(param2)) 
        && (param1 < param2) ) {
...
//second version with integer numbers
function randomNum (param1,param2) {
  if ( (typeof param1 === 'number') && (!Number.isNaN(param1)) 
        && (typeof param2 === 'number') && (!Number.isNaN(param2)) 
        && (param1 < param2) ) {
...
/*5. Да се състави функция, която приема един параметър, който е масив от числа и връща най-голямото и 
най-малкото число от тях. (Да не се използва Math.max)*/
function maxmin (arr) {
...
maxmin([12,10,4, 64, 43, 5]);


/*6. Да се състави функция, която намира най-младият човек със смартфон iPhone и да върне неговия обект */
let arr = [  
    {
